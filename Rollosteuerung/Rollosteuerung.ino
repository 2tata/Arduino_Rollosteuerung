...
#define RolloLaufzeit 20000
void setup() {

  Serial.begin(9600); // baudrade inizialisiren 
...
void loop() {
  ...
  //Drücken dess Home Buttons
  if(digitalRead(TasterHome) == LOW) //Prüft nach ob die Home Taste gedrückt wird
  {  
    Homeposition();
  }

  //Drücken dess Aktion1 Buttons
...
void Homeposition()
{
  ...
  while(Home == 0)
  {
    if(digitalRead(RolloTaster1) == HIGH) //Überprüft ob das Rollo auf der Home Position ist. 
    {
...
void Aktion1()
{
  ...
  while(Aktion1 == 0)
  {
    ...
    else
    {
      digitalWrite(MotorUp1, LOW);
      digitalWrite(MotorDown1, LOW);
      Serial.println("Rolo ist nicht auf der Homeposition"); // zur kontrolle
    }
  }
}
